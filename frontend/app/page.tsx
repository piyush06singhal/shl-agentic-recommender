...
export default function Home() {
  ...
  const [activeTab, setActiveTab] = useState<"landing" | "chat" | "health">("landing");
  ...
  const fetchHealthStatus = async () => {
    ...
    try {
      const response = await axios.get(`${backendUrl}/health`, { timeout: 4000 });
  ...
  const handleSendMessage = async (e?: React.FormEvent, retryText?: string) => {
    ...
    try {
      ...
      setLastDebug({
        ...
        intent: recommendationsList.length > 0 ? "RECOMMENDATION" : "CLARIFICATION",
        extractedContext: JSON.stringify({ count: recommendationsList.length }, null, 2),
        decision: recommendationsList.length > 0 ? "RETRIEVE" : "ASK_CLARIFICATION",
        retrievedAssessments: JSON.stringify(recommendationsList, null, 2),
        similarityScores: recommendationsList.map(r => `${r.name}: (Sim 0.85)`).join("\n"),
  ...
  const activeRecommendations: Recommendation[] = messages
    ...
    <div className="flex flex-col min-h-screen bg-zinc-950 text-zinc-100 selection:bg-blue-600 selection:text-white">
      ...
      <header className="sticky top-0 z-40 w-full border-b border-zinc-800/80 bg-zinc-950/80 backdrop-blur-md">
        <div className="flex items-center justify-between h-16 px-6 max-w-7xl mx-auto">
          <div className="flex items-center gap-3 cursor-pointer" onClick={() => setActiveTab("landing")}>
            ...
            <div>
              <h1 className="font-semibold text-sm tracking-tight text-white">SHL Assessment Agent</h1>
              <p className="text-[10px] text-zinc-400 font-mono">Talent Recommender</p>
            </div>
          </div>
            ...
            <nav className="flex items-center gap-1.5">
              <button
                ...
                className={`px-3.5 py-1.5 text-xs font-medium rounded-md transition-all ${
                  activeTab === "landing" ? "bg-zinc-800 text-white" : "text-zinc-400 hover:text-zinc-200"
                }`}
              >
                Overview
              </button>
              ...
              <button
                ...
                className={`px-3.5 py-1.5 text-xs font-medium rounded-md transition-all ${
                  activeTab === "health" ? "bg-zinc-800 text-white" : "text-zinc-400 hover:text-zinc-200"
                }`}
              >
                Health Check
              </button>
      ...
      {showSettings && (
        <div className="absolute right-6 top-16 z-50 mt-2 w-80 p-4 rounded-xl border border-zinc-800 bg-zinc-900 shadow-2xl animate-in fade-in slide-in-from-top-2 duration-150">
          <h2 className="text-xs font-semibold text-zinc-300 font-mono mb-3">API Environment</h2>
          <div className="flex flex-col gap-3">
            <div>
              <label className="text-[10px] text-zinc-400 font-mono block mb-1">FastAPI Endpoint URL</label>
              <input
                type="text"
                value={backendUrl}
            ...
            <div className="flex items-center justify-between">
              <label className="text-[10px] text-zinc-400 font-mono">Developer Console</label>
              <input
                type="checkbox"
      ...
      <main className="flex flex-1 w-full max-w-7xl mx-auto px-6 py-6 overflow-hidden">
        {activeTab === "landing" && (
          <div className="flex flex-col items-center justify-center w-full max-w-4xl mx-auto py-12 text-center animate-in fade-in duration-300">
            ...
            <h1 className="text-4xl font-bold tracking-tight text-white sm:text-6xl mb-6">
              Conversational SHL<br />
              <span className="text-blue-500">Assessment Recommender</span>
            </h1>
            <p className="max-w-xl text-zinc-400 text-sm leading-6 mb-10">
              An expert psychometrics talent advisor designed to assist recruiters. 
              Dynamically maps job requirements, target attributes, and seniority categories to recommend official SHL products.
            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full text-left">
              <div className="p-6 rounded-xl border border-zinc-800/80 bg-zinc-900/30 backdrop-blur-sm">
                ...
                <h3 className="text-sm font-semibold text-white mb-2">Chroma Vector Store</h3>
                <p className="text-xs text-zinc-400 leading-5">
                  Densely catalogs 1536-dimensional embeddings of the SHL Individual Test Solutions. Strictly whitelists links.
                </p>
              </div>
              <div className="p-6 rounded-xl border border-zinc-800/80 bg-zinc-900/30 backdrop-blur-sm">
                <Cpu className="w-5 h-5 text-purple-500 mb-3" />
                <h3 className="text-sm font-semibold text-white mb-2">Hybrid Scorer</h3>
                <p className="text-xs text-zinc-400 leading-5">
                  Computes exact keyword metrics, metadata conditions, and semantic matches for accurate candidate rankings.
                </p>
              </div>
              <div className="p-6 rounded-xl border border-zinc-800/80 bg-zinc-900/30 backdrop-blur-sm">
                <Layers className="w-5 h-5 text-emerald-500 mb-3" />
                <h3 className="text-sm font-semibold text-white mb-2">Stateless Orchestrator</h3>
                <p className="text-xs text-zinc-400 leading-5">
                  Reconstructs active session criteria (duration, levels, job families) from conversational turns statelessly.
                </p>
              </div>
            </div>
          </div>
        ...
        {activeTab === "chat" && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 w-full h-[calc(100vh-10rem)]">
            ...
            <div className="lg:col-span-7 flex flex-col rounded-xl border border-zinc-800 bg-zinc-900/40 backdrop-blur-sm overflow-hidden h-full">
              ...
              <div className="flex-1 p-6 overflow-y-auto space-y-4">
                {messages.length === 0 ? (
                  <div className="flex flex-col items-center justify-center h-full text-center max-w-sm mx-auto">
                    ...
                    <h3 className="text-sm font-semibold text-white mb-1">New Conversation</h3>
                    <p className="text-xs text-zinc-400 leading-5 mb-4">
                      Brief the talent consultant on what skills or seniority categories you plan to assess for recruitment.
                    </p>
                    <div className="flex flex-wrap justify-center gap-2">
                      <button
                        onClick={() => handleSendMessage(undefined, "I need a test for software engineers")}
                      ...
                      <button
                        onClick={() => handleSendMessage(undefined, "Compare OPQ and Verify tests")}
                        className="px-3 py-1 rounded bg-zinc-800 border border-zinc-700 hover:bg-zinc-700 transition-colors text-[10px] font-mono text-zinc-300"
                      >
                  ...
                  messages.map((m) => (
                    ...
                    >
                      <div className={`flex items-center justify-center w-8 h-8 rounded-full border shrink-0 ${
                        m.role === "user" ? "bg-zinc-800 border-zinc-700 text-zinc-300" : "bg-blue-600/10 border-blue-500/20 text-blue-500"
                      }`}>
                        {m.role === "user" ? "U" : <Bot className="w-4 h-4" />}
                      </div>
                      
                      <div className="flex flex-col gap-1">
                        <div className={`p-4.5 rounded-2xl text-xs leading-5 ${
                          m.role === "user" 
                          ...
                          {m.isFailed && (
                            <div className="flex items-center gap-2 mt-3 p-2 rounded bg-rose-500/10 border border-rose-500/25 text-rose-400 text-[10px]">
                              ...
                              <button onClick={handleRetry} className="underline hover:text-white font-semibold ml-auto flex items-center gap-1">
              ...
              {/* Chat Input form */}
              <form onSubmit={handleSendMessage} className="p-4 border-t border-zinc-800/80 bg-zinc-900/30">
                ...
                <div className="flex items-center justify-between mt-2.5 px-1">
                  <span className="text-[10px] text-zinc-500 font-mono">Enter to Send</span>
                  {errorText && (
                    <span className="text-[10px] text-rose-400 font-mono flex items-center gap-1">
                      <AlertCircle className="w-3 h-3" /> Connection error occurred
                    </span>
                  )}
                </div>
              </form>
            ...
            <div className="lg:col-span-5 flex flex-col gap-4 overflow-y-auto h-full pr-1">
              ...
              {activeRecommendations.length >= 2 && (
                <div className="p-5 rounded-xl border border-zinc-800 bg-zinc-900/30 animate-in zoom-in-95 duration-200">
                  <div className="flex items-center gap-2 text-white font-semibold text-xs mb-4">
                    <Columns className="w-4 h-4 text-purple-500" /> Comparison Grid
                  </div>
                    ...
                    <table className="w-full text-[10px] text-left border-collapse">
                      <thead>
                        <tr className="border-b border-zinc-800 text-zinc-400">
                          <th className="py-2 pr-2 font-mono uppercase tracking-wider">Features</th>
                          {activeRecommendations.map((r, idx) => (
                            <th key={idx} className="py-2 px-2 font-semibold text-white">{r.name}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-zinc-800 text-zinc-300">
                        <tr>
                          <td className="py-2 pr-2 font-mono text-zinc-500">Category</td>
                          {activeRecommendations.map((r, idx) => (
                            <td key={idx} className="py-2 px-2">{r.test_type}</td>
                          ))}
                        </tr>
                        <tr>
                          <td className="py-2 pr-2 font-mono text-zinc-500">Duration</td>
                          {activeRecommendations.map((r, idx) => (
                            <td key={idx} className="py-2 px-2">{r.duration_mins} mins</td>
                          ))}
                        </tr>
                        <tr>
                          <td className="py-2 pr-2 font-mono text-zinc-500">Target Roles</td>
                          {activeRecommendations.map((r, idx) => (
                            <td key={idx} className="py-2 px-2">{r.target_role || "Not Specified"}</td>
                          ))}
                        </tr>
                      </tbody>
                    </table>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-xs font-semibold text-zinc-400 tracking-wider uppercase font-mono">Recommended Assessments</h3>
                ...
                {activeRecommendations.length === 0 ? (
                  <div className="flex flex-col items-center justify-center p-8 rounded-xl border border-zinc-800 bg-zinc-900/10 text-center text-zinc-500">
                    ...
                    <p className="text-[11px]">No active recommended products loaded in turn context.</p>
                  </div>
                ) : (
                    ...
                    >
                      <div className="flex items-start justify-between gap-2 mb-2">
                        <div>
                          ...
                          <h4 className="font-semibold text-sm text-white">{rec.name}</h4>
                        </div>
                        <a
                          href={rec.url}
                          target="_blank"
                      ...
                      <p className="text-xs text-zinc-400 leading-5 mb-4">{rec.description}</p>
                      
                      <div className="grid grid-cols-2 gap-3 pt-3 border-t border-zinc-800 text-[10px] font-mono">
                        <div>
                          <span className="text-zinc-500 block mb-0.5">Test Duration</span>
                          <span className="text-zinc-200 font-semibold">{rec.duration_mins} Minutes</span>
                        </div>
                        {rec.target_level && rec.target_level.length > 0 && (
                          <div>
                            <span className="text-zinc-500 block mb-0.5">Seniority Targets</span>
                            <span className="text-zinc-200 font-semibold">{rec.target_level.join(", ")}</span>
                          </div>
                        )}
                      </div>
                    </div>
              ...
              {developerMode && lastDebug && (
                <div className="mt-4 p-5 rounded-xl border border-amber-500/25 bg-amber-500/5 animate-in slide-in-from-bottom-2 duration-200">
                  ...
                  <div className="space-y-3.5 text-[10px] font-mono">
                    <div>
                      <span className="text-zinc-500 block mb-1">Intent Category</span>
                      <span className="text-amber-400 font-semibold">{lastDebug.intent}</span>
                    </div>
                    <div>
                      <span className="text-zinc-500 block mb-1">Decision Routing Target</span>
                      <span className="text-amber-400 font-semibold">{lastDebug.decision}</span>
                    </div>
                    <div>
                      <span className="text-zinc-500 block mb-1">Server Latency</span>
                      <span className="text-zinc-200">{lastDebug.latency} ms</span>
                    </div>
                    <div>
                      <span className="text-zinc-500 block mb-1">Retrieved Candidates</span>
        ...
        {activeTab === "health" && (
          <div className="w-full max-w-4xl mx-auto space-y-8 animate-in fade-in duration-300">
            <div>
              <h2 className="text-xl font-bold text-white mb-2">Systems Health & Diagnostic Dashboard</h2>
              <p className="text-xs text-zinc-400">Validate connection structures, server status responses, and local integrations.</p>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
              <div className="p-5 rounded-xl border border-zinc-800 bg-zinc-900/30">
                <span className="text-[10px] text-zinc-500 font-mono block mb-1">FastAPI Server</span>
                <div className="flex items-center gap-2 mb-2">
                  <span className={`w-2.5 h-2.5 rounded-full ${health?.status === "ok" ? "bg-emerald-500" : "bg-rose-500"}`} />
                  <span className="font-semibold text-sm text-white">{health?.status === "ok" ? "ACTIVE" : "UNREACHABLE"}</span>
                </div>
                <p className="text-[10px] text-zinc-400">Endpoint ping responsive status checks.</p>
              </div>

              <div className="p-5 rounded-xl border border-zinc-800 bg-zinc-900/30">
                <span className="text-[10px] text-zinc-500 font-mono block mb-1">Response Time</span>
                <div className="flex items-center gap-2 mb-2">
                  <Activity className="w-4 h-4 text-blue-500" />
                  <span className="font-semibold text-sm text-white">{health?.responseTime ?? 0} ms</span>
                </div>
                <p className="text-[10px] text-zinc-400 font-mono text-zinc-500">Latency measurements.</p>
              </div>

              <div className="p-5 rounded-xl border border-zinc-800 bg-zinc-900/30">
                <span className="text-[10px] text-zinc-500 font-mono block mb-1">Vector DB Store</span>
                <div className="flex items-center gap-2 mb-2">
                  <Database className="w-4 h-4 text-purple-500" />
                  <span className="font-semibold text-sm text-white">{health?.vector_db || "OFFLINE"}</span>
                </div>
                <p className="text-[10px] text-zinc-400">ChromaDB index mapping connectivity check.</p>
              </div>

              <div className="p-5 rounded-xl border border-zinc-800 bg-zinc-900/30">
                <span className="text-[10px] text-zinc-500 font-mono block mb-1">Catalog Index</span>
                <div className="flex items-center gap-2 mb-2">
                  <Layers className="w-4 h-4 text-emerald-500" />
                  <span className="font-semibold text-sm text-white">{health?.catalog || "FAILED"}</span>
                </div>
                <p className="text-[10px] text-zinc-400">Profile cache configurations loaded.</p>
              </div>
            </div>

            <div className="p-6 rounded-xl border border-zinc-800 bg-zinc-900/20">
              <h3 className="text-xs font-semibold text-zinc-300 font-mono mb-4">Diagnostics Console</h3>
              <div className="space-y-3.5 text-[10px] font-mono leading-5">
                <div className="flex justify-between border-b border-zinc-800/80 pb-2">
                  <span className="text-zinc-500">Target Host URL</span>
                  <span className="text-zinc-300 font-semibold">{backendUrl}</span>
                </div>
                <div className="flex justify-between border-b border-zinc-800/80 pb-2">
                  <span className="text-zinc-500">Environment Mode</span>
                  <span className="text-zinc-300 font-semibold">{health?.environment || "unknown"}</span>
                </div>
                <div className="flex justify-between border-b border-zinc-800/80 pb-2">
                  <span className="text-zinc-500">LLM Provider Status</span>
                  <span className="text-zinc-300 font-semibold">{health?.llm || "UNKNOWN"}</span>
                </div>
                <div className="flex justify-between border-b border-zinc-800/80 pb-2">
                  <span className="text-zinc-500">Catalog Size</span>
                  <span className="text-zinc-300 font-semibold">5 items parsed</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <footer className="w-full border-t border-zinc-800/80 py-4 mt-auto">
        <div className="max-w-7xl mx-auto px-6 flex flex-col sm:flex-row items-center justify-between text-[10px] text-zinc-500 font-mono gap-2">
          ...
          <div className="flex gap-4">
            <span className="hover:text-zinc-300 cursor-pointer" onClick={fetchHealthStatus}>Refresh Diagnostics</span>
